const county_districts_selection = document.querySelector(
  "#county-districts-selection"
);
const county_households_selection = document.querySelector(
  "#county-houses-selection"
);
const raw_households_data = [];
const raw_Population_data = [];
fetch("./census-2008.json")
  ...
  .then((data) => {
    ...
    const female = CalculateTotalFemale(POPULATION);
    //Computing total male
    const male = CalculateTotalMale(POPULATION);
    ...
    const population_per_county = PopulationPerCounty(POPULATION);

    //get the array of counties without duplicate for the label on the chart from population data
    const population_county = GetCountiesWithoutDuplicate(POPULATION);

    //Counties in select tag
    GetDropdownOfCounties(population_county, county_districts_selection);

    const househood_counties = GetCountiesForHouseHolds(HOUSEHOLD);
    ...
    PutHouseHoldsCountiesInSelect(
      househood_counties,
      county_households_selection
    );
...
function CalculateTotalFemale(populationData) {
  let total_female = populationData.reduce(
    (acc, value) => acc + value.female,
    0
  );
  //Print total  Female
  const FEMALE = new Intl.NumberFormat().format(total_female);
  document
    .getElementById("femaleData")
    .insertAdjacentHTML("beforebegin", ` <b>${FEMALE}</b>`);
  return total_female;
}
//Computing total male
function CalculateTotalMale(populationData) {
  ...
  document
    .getElementById("mal")
    .insertAdjacentHTML("beforebegin", ` <b>${MALE}</b>`);
  return total_male;
}
//Computing total population
function ComputeTotalPopulation(male, female) {
  ...
  document
    .getElementById("popu")
    .insertAdjacentHTML("afterbegin", `<b>${POPULATION_TOTAL}</b>`);
...
function PopulationPerCounty(populationData) {
  let population_per_county = populationData.reduce(
...
function ReturnMaleAndFemaleAsArrays(county_data) {
  let county_male = county_data.reduce(
    (a, c) => ((a[c.county] = (a[c.county] || 0) + c.male), a),
    {}
  );
  let county_female = county_data.reduce(
    (a, c) => ((a[c.county] = (a[c.county] || 0) + c.female), a),
    {}
  );
  ...
  county.map((element) => {
    for (const key in element) {
      ...
      if (male_gender > 110000) {
        document.querySelector("#populate").insertAdjacentHTML(
          "afterend",
          `
        <tr>
            <th>${key}</th>
